...
function Apply() {
  const [isLoading, setIsLoading] = useState(true); const formRef = useRef<HTMLDivElement>(null); 
  ...
  useEffect(() => {
    const script = document.createElement("script"); script.src = "//embed.typeform.com/next/embed.js"; script.async = true; document.body.appendChild(script);
...
//   const handleClick = () => { router.push('/apply') } // Close mobile menu when route changes 
//  useEffect(() => { setIsOpen(false) }, [location])

//   useEffect(() => { const handleScroll = () => { setScrolled(window.scrollY > 10) } 
...
//    [])
  ...
  <div className="relative flex flex-col overflow-y-auto h-screen">
    ...
    {/* <motion.header
      className={`fixed top-3 left-0 right-0 z-50 flex items-center justify-center transition-all duration-300 ${scrolled ? "bg-transparent backdrop-blur-sm !text-white shadow-sm" : "bg-transparent"
        }`}
      initial={{ y: 0 }}
      animate={{ y: 0 }}
    ...
    >
      <div className="container mt-4  mx-auto flex justify-between w-[90%] items-center">
        <Link href="/" className="flex items-center justify-center ml-4 gap-2">
          <img src="/Logo.png" alt="logo" className="w-16 md:w-22" />
    ...
    {/* Main Content */}
    <div className="flex-grow relative z-10 flex items-center justify-center overflow-scroll">
